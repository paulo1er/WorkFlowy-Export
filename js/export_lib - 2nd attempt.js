var exportLib = (function() {
	// private method
	var hasChild, getElement, toc2, nodesTreeToText, test;
	...
	nodesTreeToText = function(nodes, index, level, option, indent_chars, prefix_indent_chars) {
		var start = 0;

		var indent = "";
		var output="";

		console.log("nodesTreeToText -- processing nodes["+index.toString()+"]");
		...
		// Update output
		if (nodes[index].type == "node") {
			output = indent + nodes[index].title + "\n";
		}
		if (nodes[index].note !== '') {
			output = output + indent + "[" + nodes[index].note + "]\n";
		...
		for (var i = 0; i < nodes[index].children.length; i++)
			output = output + nodesTreeToText(nodes, nodes[index].children[i], level+1, option, indent_chars, prefix_indent_chars);

		return output;
	};


	test = function(nodes, index, option, indent_chars, prefix_indent_chars) {
		return "hello";
	};
	...
	return {
		...
		toText: function(nodes, option, indent_chars, prefix_indent_chars) {
			var text = nodes[0].title + "\n";
			var previous = null;
			var prevElement = null;
			var level = 2;
